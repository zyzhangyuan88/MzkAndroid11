package com.common.base.utils

import android.graphics.Color
import android.text.TextUtils
import java.math.BigDecimal
import java.math.RoundingMode
import java.text.DecimalFormat
import java.util.*

/**
 * 一些额外的工具类，不好分类的那种
 *
 * @author Alex  2022/4/7
 */
object CommonUtil {

    /** 判断String是否为空或空串，主要提供给xml中dataBinding用 */
    @JvmStatic
    fun isEmpty(str: String?): Boolean {
        return str?.isEmpty() ?: true
    }

    /** 获取随机rgb颜色值 */
    fun randomColor(): Int {
        Random().run {
            //0-190, 如果颜色值过大,就越接近白色,就看不清了,所以需要限定范围
            val red = nextInt(190)
            val green = nextInt(190)
            val blue = nextInt(190)
            //使用rgb混合生成一种新的颜色,Color.rgb生成的是一个int数
            return Color.rgb(red, green, blue)
        }
    }


    @JvmStatic
    fun priceFormat(price: String): String {
        return if (TextUtils.isEmpty(price) || TextUtils.equals(
                rvZeroAndDot(
                    price
                ), "0"
            )
        ) {
            "价格面议"
        } else {
            "¥" + getNumberWanTwo(price) + "万"
        }
    }


    fun rvZeroAndDot(valuex: String?): String? {
        var valuex = valuex
        if (valuex != null && valuex.indexOf(".") > 0) {

            // 去掉多余的0
            valuex = valuex.replace("0+?$".toRegex(), "")

            // 如最后一位是.则去掉
            valuex = valuex.replace("[.]$".toRegex(), "")
        }
        return valuex
    }


     fun getNumberWanTwo(value: String?): String? {
        val bigDecimal = BigDecimal(value)
        val decimal = bigDecimal.divide(BigDecimal("10000"))
        val formater = DecimalFormat("#0.00")
        formater.roundingMode = RoundingMode.HALF_UP
        return formater.format(decimal)
    }
}